...
1. Quadratic Penalty Method(QPM) : `qpm(f, ce, ci, x0, inner_opt, tol)`  
   1) The output of QPM contains log of $x_k, \quad f\(x_k\), \quad ∇f\(x_k\), \quad max\(|c_e\(x_k\)|\), \quad max\(|c_i\(x_k\)|\)$  
3. Augmented Lagrangian Method(ALM) : `alm(f, ce, ci, x0, inner_opt, tol)`  
   ...
   3) The output of ALM contains log of $x_k, \quad f\(x_k\), \quad ∇L\(x_k\), \quad max\(|c_e\(x_k\)|\), \quad max\(|c_i\(x_k\)|\), \quad\\lambda_k, \quad \\nu_k$  
