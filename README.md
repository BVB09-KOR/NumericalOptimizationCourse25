...
Users simply define a single scalar objective function and list of equality/inequality constraint functions and provide them as input arguments to desired optimizer with proper initial guess point `x0`(convergence tolerance `tol` is supplementary).  
 - `module_opt.py ` 
...
1. Quadratic Penalty Method(QPM)  
   : `qpm(f, ce, ci, x0, inner_opt, tol)`  
   1) The output of QPM contains log of  
      $x_k, \quad f\(x_k\), \quad ∇f\(x_k\), \quad max\(|c_e\(x_k\)|\), \quad max\(|c_i\(x_k\)|\)$  
3. Augmented Lagrangian Method(ALM)  
   : `alm(f, ce, ci, x0, inner_opt, tol)`  
...
5. Sequential Quadratic Programming(SQP)  
   ...
   2) QPk is solved using `alm4sqp(f, ce, ci, x0, lmbda0, nu0, inner_opt, tol)`  
      - `alm4sqp()` receives $p_{k-1}^\*, \quad \lambda_{k-1}^\*, \quad \nu_{k-1}^\*$ from SQP loop as initial guess of design variables(`x0`), and lagrange multipliers `lmbda0`, `nu0`.  
   4) The output of SQP contains log of  
      $x_k, \quad f\(x_k\), \quad ∇f\(x_k\), \quad ∇L\(x_k\), \quad max\(|c_e\(x_k\)|\), \quad max\(|c_i\(x_k\)|\), \quad \lambda_k, \quad \nu_k$  
