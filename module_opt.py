...
def steplength_merit(k, mu, f, ce, ci, x_k, p_k, grad_f_k, B_k, lmbda_k, nu_k):
    ...
    sum_grad_ce = np.array([-grad_centraldiff(ce_j, x_k) if ce_j(x_k) < 0.0 else grad_centraldiff(ce_j, x_k) for ce_j in ce]).sum(axis=0)
    sum_grad_ci = np.array([-grad_centraldiff(ci_j, x_k) if ci_j(x_k) < 0.0 else np.zeros_like(x_k) for ci_j in ci]).sum(axis=0)
...
def quasi_newton_bfgs(f, x0, tol):
    ...
    for k in range(1000):
        ...
        if k >= 2:
            if ((r_grad_f <= tol) and # ‖∇f‖도 충분히 정칙점에 도달했고
    ...
    for k in np.arange(100): # max iteration of outer loop = 100
        ...
        if k == 0:
            ...
            if ((np.abs(sBs) > 1e-3) & (np.abs(sr) > 1e-3)):
                ...
                B_k = B_k
        Q_QPk = lambda p : .5*p@(B_k@p) + grad_f_k@p # Objective function of QPk subproblem
        ce_QPk = [lambda p, j=j, a=grad_ce_k_j: a@p + ce_k[j] for j, grad_ce_k_j in enumerate(grad_ce_k)] # Equality constraint function of QPk subproblem ... 클로저 late binding 문제 조심
        ci_QPk = [lambda p, j=j, a=grad_ci_k_j: a@p + ci_k[j] for j, grad_ci_k_j in enumerate(grad_ci_k)] # Inequality constraint function of QPk subproblem ... 클로저 late binding 문제 조심
        ...
        #     log_inter = alm4sqp(Q_QPk, ce_QPk, ci_QPk, p0_QPk, lmbda_k, nu_k, inner_opt, tol_inter)
        log_inter = alm4sqp(Q_QPk, ce_QPk, ci_QPk, p0_QPk, lmbda_k, nu_k, inner_opt, tol_inter) # Intermediate loop(ALM) ... 디버깅용
        ...
        if k == 0:
            mu = 1
        alpha, mu = steplength_merit(k, mu, f, ce, ci, x_k, p_new, grad_f_k, B_k, lmbda_k=lmbda_new, nu_k=nu_new) # step length alpha_k
